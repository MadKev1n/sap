...
check_file() {
    ...
    if [[ ! -f "$file" ]]; then
        ...
        log_action "INFO" "Файл $file найден" "$LOG_FILE"
    fi
}
...
setup_alias() {
    if ! grep -q "alias sap=" ~/.bashrc; then
        ...
        log_action "INFO" "Алиас sap уже существует" "$LOG_FILE"
    fi
}
    ...
    case "$action" in
        on)
            if ! iptables -t nat -C POSTROUTING -o "$interface" -j MASQUERADE 2>/dev/null; then
                ...
                log_action "INFO" "Маскарад уже включен" "$LOG_FILE"
            fi
            ;;
        off)
            if iptables -t nat -C POSTROUTING -o "$interface" -j MASQUERADE 2>/dev/null; then
                ...
                log_action "INFO" "Маскарад уже выключен" "$LOG_FILE"
            fi
            ;;
        *)
            echo -e "${RED}Используйте 'on' или 'off'${NC}" >&2
            log_action "ERROR" "Некорректный аргумент для маскарада: $action" "$LOG_FILE"
            ;;
    esac
}
...
change_ip() {
    ...
    if [[ "$new_ip" =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
        ...
        log_action "ERROR" "Некорректный IP: $new_ip" "$LOG_FILE"
    fi
}
...
add_game_from_default() {
    ...
    if [ ${#games[@]} -eq 0 ]; then
        ...
        log_action "ERROR" "Файл $DEFAULT_FILE пуст или не содержит игр" "$LOG_FILE"
        return 1
    fi
    for i in "${!games[@]}"; do
        echo "$((i+1))) ${games[$i]}"
    ...
    if [[ "$game_choice" =~ ^[0-9]+$ ]] && [ "$game_choice" -ge 1 ] && [ "$game_choice" -le "${#games[@]}" ]; then
        ...
        if ! grep -q "^$selected_game," "$APP_LIST_FILE"; then
            echo "$selected_game,\"$selected_game\"" >> "$APP_LIST_FILE" || {
                echo -e "${RED}Ошибка добавления игры $selected_game${NC}" >&2
                log_action "ERROR" "Ошибка добавления игры $selected_game" "$LOG_FILE"
                return 1
            }
            echo -e "${GREEN}Добавлена игра '$selected_game' в $APP_LIST_FILE${NC}"
            log_action "INFO" "Добавлена игра $selected_game в $APP_LIST_FILE" "$LOG_FILE"
        fi
        ports_added=0
            ...
            if [[ -n "$port" ]]; then
                ...
                if ! grep -q "^$selected_game,$protocol_port," "$APP_LIST_FILE"; then
                    echo "$selected_game,$protocol_port,\"$description\"" >> "$APP_LIST_FILE" || {
                        echo -e "${RED}Ошибка добавления порта $protocol_port для $selected_game${NC}" >&2
                        log_action "ERROR" "Ошибка добавления порта $protocol_port для $selected_game" "$LOG_FILE"
                        return 1
                    }
                    ((ports_added++))
                    echo -e "${GREEN}Добавлен порт '$protocol_port' для '$selected_game'${NC}"
                    log_action "INFO" "Добавлен порт $protocol_port для $selected_game" "$LOG_FILE"
                fi
            fi
        done <<< "$game_ports"
        if [ $ports_added -gt 0 ]; then
            ...
            log_action "INFO" "Новых портов для $selected_game не добавлено" "$LOG_FILE"
        fi
    else
        echo -e "${RED}Некорректный выбор игры${NC}" >&2
        log_action "ERROR" "Некорректный выбор игры: $game_choice" "$LOG_FILE"
    fi
}
...
add_game() {
    ...
    if grep -q "^$new_game," "$APP_LIST_FILE"; then
        ...
        log_action "ERROR" "Игра $new_game уже существует" "$LOG_FILE"
        return 1
    fi
...
add_port() {
    ...
    if [ ${#games[@]} -eq 0 ]; then
        ...
        log_action "ERROR" "Нет игр в $APP_LIST_FILE" "$LOG_FILE"
        return 1
    fi
    for i in "${!games[@]}"; do
        echo "$((i+1))) ${games[$i]}"
    ...
    if [[ "$game_choice" =~ ^[0-9]+$ ]] && [ "$game_choice" -ge 1 ] && [ "$game_choice" -le "${#games[@]}" ]; then
        ...
        if [[ "$protocol" =~ ^(TCP|UDP)$ ]] && [[ "$port_number" =~ ^[0-9]+$ ]] && [ "$port_number" -ge 1 ] && [ "$port_number" -le 65535 ]; then
            ...
            if grep -q "^$selected_game,$port," "$APP_LIST_FILE"; then
                ...
                log_action "ERROR" "Порт $port уже добавлен для $selected_game" "$LOG_FILE"
            else
            ...
            log_action "ERROR" "Некорректный протокол или порт: $port" "$LOG_FILE"
        fi
    else
        echo -e "${RED}Некорректный выбор игры${NC}" >&2
        log_action "ERROR" "Некорректный выбор игры: $game_choice" "$LOG_FILE"
...
remove_port() {
    ...
    if [ ${#ports[@]} -eq 0 ]; then
        ...
        log_action "ERROR" "Нет портов для удаления" "$LOG_FILE"
        return 1
    fi
    for i in "${!ports[@]}"; do
    ...
    if [[ "$port_choice" =~ ^[0-9]+$ ]] && [ "$port_choice" -ge 1 ] && [ "$port_choice" -le "${#ports[@]}" ]; then
        ...
        log_action "ERROR" "Некорректный выбор порта: $port_choice" "$LOG_FILE"
    fi
}
...
show_app_list() {
    ...
    if [[ ! -s "$APP_LIST_FILE" ]]; then
        ...
        log_action "ERROR" "Файл $APP_LIST_FILE пуст" "$LOG_FILE"
        return 1
    fi
    current_game=""
    while IFS= read -r line; do
...
apply_iptables() {
    ...
    if [ -z "$ip_address" ]; then
        ...
        log_action "ERROR" "IP-адрес не задан" "$LOG_FILE"
        return 1
    fi
    ...
    while true; do
        ...
        case "$choice" in
            ...
            *)
                ...
                log_action "ERROR" "Неверный выбор в меню: $choice" "$LOG_FILE"
                ;;
        esac
